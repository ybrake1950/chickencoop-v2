...
_CONFIG_REQUIRED = ["coop_id", "temperature_threshold", "humidity_threshold", "recording_duration"]
_CONFIG_TYPES = {
    "coop_id": str,
...
class SchemaValidator:
    ...
    def validate_config(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        for key in _CONFIG_REQUIRED:
            if key not in config:
                return ValidationResult(valid=False, error=f"Missing required field: {key}")
    ...
    def validate_api_request(self, endpoint: str, body: Dict[str, Any], strict: bool = False) -> ValidationResult:
        ...
        for req in schema.get("required", []):
            if req not in body:
                return ValidationResult(valid=False, error=f"Missing required field: {req}")

        for key, prop in schema.get("properties", {}).items():
            if key in body and not isinstance(body[key], prop["type"]):
                return ValidationResult(valid=False, error=f"Type mismatch for {key}: expected {prop['type'].__name__}")

        if strict:
            allowed = set(schema.get("properties", {}).keys())
            extra = set(body.keys()) - allowed
            if extra:
                return ValidationResult(valid=False, error=f"Extra unknown fields: {', '.join(sorted(extra))}")
...
_SQL_PATTERNS = [
    re.compile(r"(\b(SELECT|INSERT|UPDATE|DELETE|DROP|ALTER|CREATE|UNION)\b)", re.IGNORECASE),
...
class InputValidator:
    ...
    def validate_username(self, username: str) -> ValidationResult:
        ...
        for pattern in _SQL_PATTERNS:
            if pattern.search(username):
                return ValidationResult(valid=False, error="Invalid characters detected (possible injection)")
    ...
    def validate_search_term(self, term: str) -> ValidationResult:
        ...
        for pattern in _SQL_PATTERNS:
            if pattern.search(term):
                return ValidationResult(valid=False, error="Invalid search term (possible injection)")
        return ValidationResult(valid=True)
    ...
    def validate_file_path(self, path: str, allow_absolute: bool = True) -> ValidationResult:
        """Validate a file path for null bytes and traversal attempts."""
        if "\x00" in path or "%00" in path:
            return ValidationResult(valid=False, error="Invalid path: null byte detected")
        if ".." in path:
            return ValidationResult(valid=False, error="Invalid path: traversal detected")
        if not allow_absolute:
            if path.startswith("/") or (len(path) >= 2 and path[1] == ":"):
                return ValidationResult(valid=False, error="Invalid path: absolute paths not allowed")
        return ValidationResult(valid=True)

    def validate_file_access(self, path: str, allowed_directory: str, follow_symlinks: bool = True) -> ValidationResult:
        """Validate that a file path resolves within the allowed directory."""
        if os.path.islink(path):
            if not follow_symlinks:
                return ValidationResult(valid=False, is_symlink=True, error="Symlink detected")
    ...
    def build_safe_command(self, base_command: List[str], user_args: List[str]) -> List[str]:
        ...
        for key, value in command.items():
            if isinstance(value, str):
                ...
                if dangerous.search(value):
                    return ValidationResult(valid=False, error=f"Invalid characters in {key}")
