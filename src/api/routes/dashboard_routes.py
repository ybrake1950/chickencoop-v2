...
_door_check_times: Dict[str, datetime] = {}
...
def get_auto_refresh():
    ...
    """
    return jsonify({"enabled": True, "interval_seconds": 30})
...
def check_door():
    ...
    return jsonify({"door_status": "closed", "cooldown": False})
...
def get_sensor_data():
    ...
    readings = [{"timestamp": now.isoformat(), "temperature": 72.5, "humidity": 65.0}]
...
def get_videos():
    ...
    _date_range = request.args.get("date_range")
    _sort = request.args.get("sort", "newest")
    ...
    return jsonify(
        {"videos": videos, "total": len(videos), "limit": limit, "offset": offset}
    )
...
def get_video_url(video_id):
    ...
    return jsonify({"url": url, "expires_in": 3600, "video_id": video_id})
...
def manual_record():
    ...
    return (
        jsonify({"status": "recording_started", "coop_id": coop_id, "camera": camera}),
        202,
    )
...
def export_sensor_data():
    ...
    _coop_id = request.args.get("coop_id")
